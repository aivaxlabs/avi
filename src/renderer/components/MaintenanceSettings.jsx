import { Archive, ChevronLeft, ChevronRight, Database, HardDrive, RotateCcw, Search, Trash2 } from 'lucide-react';
...
export function MaintenanceSettings() {
  ...
  const tabs = (
    <div className="maintenance-tabs" role="tablist" aria-label="Maintenance views">
      <button type="button" role="tab" aria-selected={tab === 'archive'} onClick={() => setTab('archive')}>
        Archive
      </button>
      <button type="button" role="tab" aria-selected={tab === 'semaphores'} onClick={() => setTab('semaphores')}>
        Semaphores
      </button>
    </div>
  );
      ...
      <div className="maintenance-settings">
        ...
        {tab === 'archive' ? (
          <section className="settings-section archive-settings">
            ...
            {error && (
              <div className="settings-context-error" role="alert">
                {error}
              </div>
            )}
          </section>
          ...
          <section className="settings-section">
            ...
            <div className={`settings-section-card archive-list${listLoading ? ' is-loading' : ''}`} aria-busy={listLoading}>
              {conversations.length ? (
                ...
                <div className="settings-empty archive-empty">{query ? 'No archived conversations match your search.' : 'No archived conversations.'}</div>
              )}
            </div>
            {state.pagination.total > 0 && (
              <nav className="archive-pagination" aria-label="Archive pages">
                <span>
                  {state.pagination.total} archived conversation
