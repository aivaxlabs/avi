...
4. **Use the v1 shape.** Produce an ESM entrypoint named `plugin.js` with a default definition object or async factory receiving `{ apiVersion, definePlugin }`. Set definition `apiVersion: 1`, `id`, `name`, and a strict semantic `version` such as `1.0.0`. Use lowercase kebab-case IDs. Package optional supporting files beside the entrypoint in a `.zip` only when necessary.
5. **Implement minimally.** Add only requested contribution arrays: `context`, `mcps`, `tools`, `auxiliaryPanels`, `themes`, `personalities`, or `providers`. Keep descriptor data plain and serializable and use only each contribution type's documented top-level handlers; unknown fields and unsupported function keys are rejected. Do not invent settings schemas, plugin-defined lifecycle/disposal hooks, update APIs, or renderer JavaScript. Avi itself manages file-based enable, disable, and removal.
...
8. **Review the result.** Confirm the `plugin.js` entrypoint, necessary package files only, API compatibility, ID collisions, serializability, handler placement, no credentials, no arbitrary renderer code, correct tool approval fields, provider stream events, and failure behavior. Review the final diff.
