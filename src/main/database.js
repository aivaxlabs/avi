...
function childThreadContext(conversation) {
  ...
  return [{
    ...
    content: [
      ...
      'By default, explore and investigate: answer questions, explain the current implementation, and analyze what the parent thread and its sub-agents are doing. Prefer proposing that actions, changes, and implementations be delegated to the parent thread rather than executing them here.',
      'This is a preference, not a read-only restriction. If the user explicitly asks you to perform work in this side chat, carry it out here using the available tools, including file changes, scripts, commands, and external operations. Do not insist on delegation or ask the user to repeat an explicit request.',
      'Stay within the requested scope and follow the tools\' availability, permission, and safety requirements. A question or discussion alone does not authorize changes or operations.',
      ...
      'Keep responses concise and prefer the forked history and focused reads. When execution is explicitly requested, complete and validate the work rather than stopping early for brevity.',
