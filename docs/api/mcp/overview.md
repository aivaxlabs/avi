...
`chat_send_prompt` keeps its existing arguments and queue/steer behavior. Delivered text is wrapped in `<cross-message from_thread_id="..." from_role="..." from_name="...">`, with escaped runtime-derived origin attributes and an explicit agent-to-agent/not-user-instruction notice. `from_role` is the source conversation type; `from_name` is omitted when unavailable. Global MCP calls without a source conversation use `external` / `external_agent`. The persisted envelope is also included in model history; old messages are not migrated. See [Sub-agents](../../Sub-agents.md#coordination-tools).
